...
#include "QAbstractProxyModel_c.h"
...
QModelIndex QAbstractProxyModel_wrapper::mapFromSource(
  ...
  if (m_mapFromSourceCallback) {
    ...
    std::cerr << "mapFromSource: Warning: Calling pure-virtual\n";
    return {};
  }
}
...
QAbstractProxyModel_wrapper::mapToSource(const QModelIndex &proxyIndex) const {
  if (m_mapToSourceCallback) {
    ...
    std::cerr << "mapToSource: Warning: Calling pure-virtual\n";
    return {};
  }
}
...
extern "C" {
void c_QAbstractProxyModel_Finalizer(void *cppObj) {
...
void *c_QAbstractProxyModel__sourceModel(void *thisObj) {
  const auto &result = fromPtr(thisObj)->sourceModel();
  return result;
}
