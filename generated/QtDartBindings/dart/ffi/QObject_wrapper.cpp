...
bool c_static_QObject__disconnect_QObject_char_QObject_char(
  ...
  return
      // tag=1068
      QtDartBindings_wrappersNS::QObject_wrapper::disconnect(sender, signal,
                                                             receiver, member);
}
...
bool c_QObject__event_QEvent(void *thisObj, void *event_) {
  ...
  return
      ...
      [&] {
        ...
        auto wrapperPtr =
            dynamic_cast<QtDartBindings_wrappersNS::QObject_wrapper *>(
                targetPtr);
        if (wrapperPtr) {
...
bool c_QObject__setProperty_char_QVariant(void *thisObj, const char *name,
  ...
  return
      // tag=1010
      fromPtr(thisObj)->setProperty(name, value);
}
